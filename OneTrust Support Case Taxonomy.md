# OneTrust Support Case Taxonomy

**Scope:** Solution / Product / Capability
...
For case creation, infer values where confidence is sufficient, ask only about unresolved values, provide valid options, show the final values in the intake draft, and obtain customer approval before submission. The [Escalation Protocol](Escalation%20Protocol.md) owns the conversation and submission workflow.
...
The chat intake form hides those three fields, so the customer can only verify them through what you say in chat first.

Use the values below when preparing or validating a support-case intake.

## Rules

- Use the exact strings as written in this document, including any unusual whitespace or curly punctuation.
